...
struct ProjectProcessor {
    ...
    ProjectProcessor(Project::Folder& folder) : folder(folder) {
        for (auto& arg : g_config->clang.excludeArgs)
            ...
            else if (Expected<GlobPattern> glob_or_err =
    ...
    void getSearchDirs(Project::Entry& entry) {
        ...
        // a weird C++ deduction guide heap-use-after-free causes libclang to
        // crash.
        ...
        CompilerInvocation::CreateFromArgs(
            *CI, CCArgs.data(), CCArgs.data() + CCArgs.size(), Diags);
...
void Project::loadDirectory(const std::string& root, Project::Folder& folder) {
    ...
    if (!cdb) {
        if (g_config->compilationDatabaseCommand.size() ||
            sys::fs::exists(path))
        ...
        if (StringRef(path).startswith(root)) {
            ...
            if (!match) {
                ...
                if (it != folder.path2entry_index.end()) {
                    ...
                    // best->compdb_size is >0 for an explicit
                    // compile_commands.json entry, 0 for an implicit entry.
                    exact_match =
                        entry.compdb_size > 0 && entry.filename == path;
                    if ((match = exact_match || can_redirect) &&
                        entry.compdb_size) {
        ...
        for (auto& [root, folder] : root2folder) {
            ...
            for (const Project::Entry& entry : folder.entries) {
                ...
                if (match.matches(entry.filename, &reason) &&
                    ...
                    bool interactive =
                        wfiles->getFile(entry.filename) != nullptr;
                    args = entry.args;
                    args.insert(args.end(), extra_args.begin(),
                                extra_args.end());
                    args.push_back(
                        intern("-working-directory=" + entry.directory));
                    pipeline::index(
                        entry.filename, args,
                        interactive ? IndexMode::Normal : IndexMode::Background,
                        false, id);
                } else {
        ...
        if (StringRef(path).startswith(root)) {
            for (const Project::Entry& entry : folder.entries) {
                ...
                if (sys::path::stem(entry.filename) == stem &&
                    ...
                    pipeline::index(entry.filename, args, IndexMode::Background,
                                    true);
            }
            break;
        }
