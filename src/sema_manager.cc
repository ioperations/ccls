...
struct ProxyFileSystem : FileSystem {
    ...
    llvm::ErrorOr<std::unique_ptr<File>> openFileForRead(
        const Twine& Path) override {
        return FS->openFileForRead(Path);
    }
    directory_iterator dir_begin(const Twine& Dir,
...
struct PreambleStatCache {
    ...
    IntrusiveRefCntPtr<llvm::vfs::FileSystem> producer(
    ...
    IntrusiveRefCntPtr<llvm::vfs::FileSystem> consumer(
        IntrusiveRefCntPtr<llvm::vfs::FileSystem> fs) {
        struct VFS : llvm::vfs::ProxyFileSystem {
            ...
            llvm::ErrorOr<llvm::vfs::Status> status(
                ...
                if (i != cache.cache.end()) return i->getValue();
...
CharSourceRange diagnosticRange(const clang::Diagnostic& d,
    ...
    for (const auto& cr : d.getRanges()) {
        ...
        if (locationInRange(loc, r, m)) return r;
    }
    ...
    auto r =
        Lexer::makeFileCharRange(CharSourceRange::getTokenRange(loc), m, l);
    if (!r.isValid())  // Fall back to location only, let the editor deal with
                       // it.
    ...
    void flush() {
        ...
        if (!mentions)
            for (auto& n : last->notes)
                if (n.concerned) mentions = true;
        if (mentions) output.push_back(std::move(*last));
        last.reset();
    }
   ...
   public:
    ...
    void HandleDiagnostic(DiagnosticsEngine::Level level,
        ...
        auto fillDiagBase = [&](DiagBase& d) {
            ...
            d.range = fromCharSourceRange(sm, *langOpts,
                                          diagnosticRange(info, *langOpts));
        ...
        if (level == DiagnosticsEngine::Note ||
            ...
            if (info.getFixItHints().size()) {
                ...
                fillDiagBase(n);
                if (concerned) last->concerned = true;
            }
        } else {
            flush();
            last = Diag();
            fillDiagBase(*last);
            if (!info.getFixItHints().empty()) addFix(true);
        }
    }
};
...
std::unique_ptr<CompilerInstance> buildCompilerInstance(
    ...
    if (!clang->hasTarget()) return nullptr;
    ...
    if (isec.size()) {
        ...
        isec[0] =
            FrontendInputFile(main, isec[0].getKind(), isec[0].isSystem());
    }
    return clang;
}
...
bool parse(CompilerInstance& clang) {
    ...
    if (!crc.RunSafely(run)) LOG_S(ERROR) << "clang crashed";
    return ok;
}
...
void buildPreamble(Session& session, CompilerInvocation& ci,
    ...
    if (oldP) {
        ...
        for (auto& include : oldP->includes)
            if (WorkingFile* wf =
                    session.wfiles->getFileUnlocked(include.first))
                ci.getPreprocessorOpts().addRemappedFile(
                    include.first,
                    llvm::MemoryBuffer::getMemBufferCopy(wf->buffer_content)
                        .release());
    }
    ...
    if (auto newPreamble = PrecompiledPreamble::Build(
        ...
        if (oldP) {
            ...
            for (auto& include : pc.includes)
                if (include.second == 0) {
                    while (it != old_includes.end() &&
                           it->first < include.first)
                        ++it;
                    if (it == old_includes.end()) break;
...
void* preambleMain(void* manager_) {
    ...
    while (true) {
        ...
        if (pipeline::g_quit.load(std::memory_order_relaxed)) break;
        ...
        if (task.comp_task) {
            ...
            int debounce = created ? g_config->diagnostics.onOpen
                                   : g_config->diagnostics.onSave;
            if (debounce >= 0) manager->scheduleDiag(task.path, debounce);
        }
    }
...
void* completionMain(void* manager_) {
    ...
    while (true) {
        std::unique_ptr<SemaManager::CompTask> task =
            manager->comp_tasks.dequeue();
        if (pipeline::g_quit.load(std::memory_order_relaxed)) break;
        ...
        IntrusiveRefCntPtr<llvm::vfs::FileSystem> fs =
            preamble ? preamble->stat_cache->consumer(session->fs)
                     : session->fs;
...
#endif
        bool in_preamble = getOffsetForPosition(
                               {task->position.line, task->position.character},
                               content) < (int)bounds.Size;
        if (in_preamble) {
            preamble.reset();
        } else if (preamble &&
                   bounds.Size != preamble->preamble.getBounds().Size) {
            manager->preamble_tasks.pushBack(
                {task->path, std::move(task), false}, true);
            continue;
        }
        ...
        if (preamble) {
            bool rebuild = false;
            {
                ...
                for (auto& include : preamble->includes)
                    if (WorkingFile* wf =
                            manager->wfiles->getFileUnlocked(include.first);
            ...
            if (rebuild) {
                manager->preamble_tasks.pushBack({task.path, nullptr, true},
                                                 true);
                continue;
            }
        }
...
void SemaManager::scheduleDiag(const std::string& path, int debounce) {
    ...
    if (flag) diag_tasks.pushBack({path, now + debounce, debounce}, false);
}

void SemaManager::onView(const std::string& path) {
    std::lock_guard lock(mutex);
    if (!sessions.get(path)) preamble_tasks.pushBack(PreambleTask{path}, true);
}
...
std::shared_ptr<ccls::Session> SemaManager::ensureSession(
    ...
    if (!session) {
        ...
        if (LOG_V_ENABLED(1)) {
            line = "\n ";
            for (auto& arg : session->file.args) (line += ' ') += arg;
        }
