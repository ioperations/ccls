...
struct Config {
    ...
    struct Cache {
        ...
        //
        // If true, $directory/a/b/c.cc.blob. If cache.directory is absolute,
        // make sure different projects use different cache.directory as they
        // would have conflicting cache files for system headers.
        bool hierarchicalPath = false;

        // After this number of loads, keep a copy of file index in memory
        // (which increases memory usage). During incremental updates, the index
        // subtracted will come from the in-memory copy, instead of the on-disk
        // file.
        //
        // The initial load or a save action is counted as one load.
        // 0: never retain; 1: retain after initial load; 2: retain after 2
        // loads (initial load+first save)
    ...
    struct Clang {
        ...
        // Translate absolute paths in compile_commands.json entries, .ccls
        // options and cache files. This allows to reuse cache files built
        // otherwhere if the source paths are different.
        //
        // This is a list of colon-separated strings, e.g. ["/container:/host"]
        //
        // An entry of "clang -I /container/include /container/a.cc" will be
        // translated to "clang -I /host/include /host/a.cc". This is simple
        // string replacement, so "clang /prefix/container/a.cc" will become
        // "clang /prefix/host/a.cc".
    ...
    struct Completion {
        ...
        // Some completion UI, such as Emacs' completion-at-point and
        // company-lsp, display completion item label and detail side by side.
        // This does not look right, when you see things like:
        //     "foo" "int foo()"
        //     "bar" "void bar(int i = 0)"
        // When this option is enabled, the completion item label is very
        // detailed, it shows the full signature of the candidate. The detail
        // just contains the completion item parent context.
        bool detailedLabel = true;

        // On large projects, completion can take a long time. By default if
        // ccls receives multiple completion requests while completion is still
        // running it will only service the newest request. If this is set to
        // false then all completion requests will be serviced.
        ...
        // completions to try to be nicer to clients that can't handle big
        // numbers of completion candidates. This behaviour can be disabled by
        // specifying false for the option. This option is the most useful for
        // LSP clients that implement their own filtering and sorting logic.
        bool filterAndSort = true;

        struct Include {
            // Regex patterns to match include completion candidates against.
            // They receive the absolute file path.
            //
            // For example, to hide all files in a /CACHE/ folder, use
            // ".*/CACHE/.*"
            std::vector<std::string> blacklist;

            // Maximum path length to show in completion results. Paths longer
            // than this will be elided with ".." put at the front. Set to 0 or
            // a negative number to disable eliding.
            int maxPathSize = 30;

            // Whitelist that file paths will be tested against. If a file path
            // does not end in one of these values, it will not be considered
            // for auto-completion. An example value is { ".h", ".hpp" }
            //
            // This is significantly faster than using a regex.
            std::vector<std::string> suffixWhitelist = {".h", ".hpp", ".hh",
                                                        ".inc"};
        ...
        // Add placeholder text. Effective only if client.snippetSupport is
        // true.
        //
        // false: foo($1)$0
    ...
    struct Diagnostics {
        ...
        //   0: immediately
        //   positive (e.g. 500): wait for 500 milliseconds. didChange requests
        //   in
    ...
    struct Index {
        // If a translation unit's absolute path matches any EMCAScript regex in
        // the whitelist, or does not match any regex in the blacklist, it will
        // be indexed. To only index files in the whitelist, add ".*" to the
        // blacklist. `std::regex_search(path, regex,
        ...
        // If false, names of no linkage are not indexed in the background. They
        // are indexed after the files are opened.
        bool initialNoLinkage = false;

        // Use the two options to exclude files that should not be indexed in
        // the background.
        std::vector<std::string> initialBlacklist;
        std::vector<std::string> initialWhitelist;

        // If a variable initializer/macro replacement-list has fewer than this
        // many lines, include the initializer in detailed_name.
        int maxInitializerLines = 5;

        // If not 0, a file will be indexed in each tranlation unit that
        // includes it.
        int multiVersion = 0;

        // If multiVersion != 0, files that match blacklist but not whitelist
        // will still only be indexed for one version.
        ...
        // changed. The file will always be reparsed if its own write time
        // changes. 0: no, 1: only during initial load of project, 2: yes
    ...
    struct Request {
        // If the document of a request has not been indexed, wait up to this
        // many milleseconds before reporting error.
    ...
    struct WorkspaceSymbol {
        ...
        int maxNum = 1000;
        // If true, workspace search results will be dynamically
        // rescored/reordered as the search progresses. Some clients do their
        // own ordering and assume that the results stay sorted in the same
        // order as the search progresses.
        bool sort = true;
    } workspaceSymbol;
...
REFLECT_STRUCT(Config::ClientCapability, diagnosticsRelatedInformation,
               hierarchicalDocumentSymbolSupport, linkSupport, hoverProvider,codeActionProvider ,
