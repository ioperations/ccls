...
std::string pathFromFileEntry(const FileEntry& file) {
    ...
    if (file.getName().startswith("/../")) {
        // Resolve symlinks outside of working folders. This handles leading
        // path components, e.g. (/lib -> /usr/lib) in
    ...
    if (normalizeFolder(ret)) return ret;
    ret = realPath(ret);
    normalizeFolder(ret);
    return ret;
}
    ...
    if (!invalid) {
        ...
        for (size_t i = 0; i < p.size();)
            if (c++, (uint8_t)p[i++] >= 128)
                while (i < p.size() && (uint8_t)p[i] >= 128 &&
                       (uint8_t)p[i] < 192)
                    i++;
    }
...
std::unique_ptr<CompilerInvocation> buildCompilerInvocation(
    ...
    // Similar to clang/tools/driver/driver.cpp:insertTargetAndModeArgs but
    // don't require llvm::InitializeAllTargetInfos().
    ...
    if (jobs.size() > 1) {
        for (auto& a : comp->getActions()) {
            ...
            if (isa<driver::BindArchAction>(a)) a = *a->input_begin();
            if (isa<driver::OffloadAction>(a)) {
                offload_compilation = true;
                break;
            }
        }
        if (!offload_compilation) return nullptr;
    }
    if (jobs.size() == 0 || !isa<driver::Command>(*jobs.begin()))
        return nullptr;

    const driver::Command& cmd = cast<driver::Command>(*jobs.begin());
    if (StringRef(cmd.getCreator().getName()) != "clang") return nullptr;
