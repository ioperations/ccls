    ...
    switch (kind) {
        case Kind::Func: {
            for (auto& [usr, _] : to_remove) {
                ...
                auto it =
                    llvm::find_if(func.def, [=](const QueryFunc::Def& def) {
        ...
        case Kind::Type: {
            for (auto& [usr, _] : to_remove) {
                ...
                auto it =
                    llvm::find_if(type.def, [=](const QueryType::Def& def) {
                        return def.file_id == file_id;
                    });
...
#define REMOVE_ADD(C, F)                                          \
    ...
    auto refDecl = [&](std::unordered_map<int, int>& lid2fid, Usr usr,
                       Kind kind, DeclRef& dr, int delta) {
        dr.file_id =
        ...
        [&](Usr usr, Kind kind,
            ...
            for (Use& use : p.first) {
                if (hint_implicit && use.role & Role::Implicit) {
                    // Make ranges of implicit function calls larger (spanning
                    // one more column to the left/right). This is hacky but
                    // useful. e.g. textDocument/definition on the
                    // space/semicolon in `A a;` or ` 42;` will take you to the
                    // constructor.
...
std::vector<SymbolRef> findSymbolsAtLocation(WorkingFile* wfile,
    ...
    // currently. If a macro expands to tokens led by a Kind::Type, the macro
    // and the Type have the same range. We want to find the macro definition
    ...
    if (symbols.size() && smallest) {
        ...
        for (size_t i = 1; i < symbols.size(); i++)
            if (!(sym.range == symbols[i].range &&
                  sym.kind == symbols[i].kind)) {
                symbols.resize(i);
                break;
            }
    }
