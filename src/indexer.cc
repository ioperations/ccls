...
struct IndexParam {
    ...
    void seenFile(FileID fid) {
        ...
        if (inserted) {
            const FileEntry* fe =
                ctx->getSourceManager().getFileEntryForID(fid);
            if (!fe) return;
            std::string path = pathFromFileEntry(*fe);
            it->second.path = path;
            it->second.mtime = fe->getModificationTime();
            if (!it->second.mtime)
                if (auto tim = lastWriteTime(path)) it->second.mtime = *tim;
            if (std::optional<std::string> content = readContent(path))
                it->second.content = *content;

            if (!vfs.stamp(path, it->second.mtime, no_linkage ? 3 : 1)) return;
            it->second.db = std::make_unique<IndexFile>(
                path, it->second.content, no_linkage);
        }
    }
    ...
    bool useMultiVersion(FileID fid) {
        ...
        if (it.second)
            if (const FileEntry* fe =
                    ctx->getSourceManager().getFileEntryForID(fid))
                it.first->second =
                    multiVersionMatcher->matches(pathFromFileEntry(*fe));
    ...
    switch (d->getKind()) {
        ...
        case Decl::Record:
            ...
            if (auto* rd = dyn_cast<RecordDecl>(d))
                if (rd->getTagKind() == TTK_Struct) kind = SymbolKind::Struct;
            return Kind::Type;
        ...
        case Decl::CXXMethod: {
            ...
            kind =
                md->isStatic() ? SymbolKind::StaticMethod : SymbolKind::Method;
            return Kind::Func;
        }
...
try_again:
    switch (tp->getTypeClass()) {
        ...
        case Type::TemplateSpecialization:
            if (specialization) *specialization = true;
            ...
            if (auto* s = dyn_cast<ClassTemplateSpecializationDecl>(r)) {
                if (!s->isExplicitSpecialization()) {
                    ...
                    if (result.is<ClassTemplateDecl*>())
                        ...
                        d = result
                                .get<ClassTemplatePartialSpecializationDecl*>();
                    continue;
                }
...
bool validateRecord(const RecordDecl* rd) {
    for (const auto* i : rd->fields()) {
        ...
        if (fqt->isIncompleteType() || fqt->isDependentType()) return false;
        if (const RecordType* childType = i->getType()->getAs<RecordType>())
            if (const RecordDecl* child = childType->getDecl())
                if (!validateRecord(child)) return false;
    }
    return true;
}
   ...
   public:
    ...
    std::string getComment(const Decl* d) {
        ...
        for (const char *p = raw.data(), *e = raw.end(); p < e;) {
            ...
            if (pad < 0) {
                // First line, detect the length of comment marker and put into
                // |pad|
                const char* begin = p;
                while (p < e &&
                       (*p == '/' || *p == '*' || *p == '-' || *p == '='))
        ...
        while (ret.size() && isspace(ret.back())) ret.pop_back();
    ...
    static void simplifyAnonymous(std::string& name) {
        for (std::string::size_type i = 0;;) {
            ...
            i++;
            if (name.size() - i > 19 &&
                name.compare(i + 10, 9, "namespace") == 0)
    ...
    void setVarName(const Decl* d, std::string_view short_name,
        ...
        if (!t.isNull() && deduced) {
            ...
            def.short_name_offset =
                str.size() + qualified.size() - short_name.size();
        ...
        if (init) {
            ...
            if (l.isMacroID() ||
                !sm.isBeforeInTranslationUnit(l, sr.getBegin()))
                return;
            ...
            def.hover = def.storage == SC_Static &&
                                strncmp(def.detailed_name, "static ", 7)
                            ? intern(("static " + t).str())
                            : intern(t.str());
        }
    }
    ...
    void collectRecordMembers(IndexType& type, const RecordDecl* rd) {
        ...
        while (stack.size()) {
            ...
            for (FieldDecl* fd : rd->fields()) {
                int offset1 =
                    offset < 0 ? -1 : int(offset + ctx->getFieldOffset(fd));
                if (fd->getIdentifier())
    ...
    bool handleDeclOccurrence(const Decl* d, index::SymbolRoleSet roles,
        ...
        if (is_def) switch (d->getKind()) {
                ...
                case Decl::Function:       // operator delete
                    if (src_loc.isFileID()) {
                        SourceRange sr = cast<FunctionDecl>(origD)
                                             ->getNameInfo()
                                             .getSourceRange();
        ...
        switch (kind) {
            ...
            case Kind::Func:
                ...
                // Mark as Role::Implicit to span one more column to the
                // left/right.
                if (!is_def && !is_decl &&
                ...
                if (is_def || is_decl) {
                    ...
                    if (getKind(dc, ls_kind) == Kind::Func)
                        db->toFunc(getUsr(dc))
                            .def.callees.push_back(
                                {loc, usr, Kind::Func, role});
                }
                break;
            case Kind::Type:
                type = &db->toType(usr);
                ...
                if ((is_def || type->def.detailed_name[0] == '\0') &&
                    ...
                    if (d->getKind() == Decl::TemplateTypeParm)
                        ...
                        setName(origD, info->short_name, info->qualified,
                                type->def);
                }
                if (is_def || is_decl) {
                    const Decl* dc = cast<Decl>(sem_dc);
            ...
            case Kind::Var:
                ...
                QualType t;
                if (auto* vd = dyn_cast<ValueDecl>(d)) t = vd->getType();
                if (is_def || is_decl) {
                    const Decl* dc = cast<Decl>(sem_dc);
                    ...
                    if (!t.isNull()) {
                        if (auto* bt = t->getAs<BuiltinType>()) {
                        ...
                        } else if (const Decl* d1 =
                                       getAdjustedDecl(getTypeDecl(t))) {
                            ...
                            if (isa<TemplateTypeParmDecl>(d1)) {
                                ...
                                if (sm.getFileID(sr1.getBegin()) == fid) {
                                    ...
                                    type1.def.spell = {
                                        Use{{fromTokenRange(sm, lang,
                                                            {sl1, sl1}),
                                             Role::Definition},
                                            lid},
                                        fromTokenRange(sm, lang, sr1)};
                                    type1.def.detailed_name =
                                        intern(info1->short_name);
                                    type1.def.short_name_size =
                                        int16_t(info1->short_name.size());
                    ...
                    if (sm.getFileID(l) == fid) {
                        var->def.spell = {
                            Use{{fromTokenRange(sm, lang, {l, l}),
                                 Role::Definition},
                                lid},
        ...
        switch (d->getKind()) {
                ...
                if (is_def) {
                    ...
                    if (rd && rd->hasDefinition())
                        for (const CXXBaseSpecifier& base : rd->bases())
                            if (const Decl* baseD = getAdjustedDecl(
                                    getTypeDecl(base.getType()))) {
                ...
                if (auto* tag_d = dyn_cast<TagDecl>(d)) {
                    if (type->def.detailed_name[0] == '\0' &&
                        ...
                        if (TypedefNameDecl* td =
                                tag_d->getTypedefNameForAnonDecl()) {
            ...
            case Decl::ClassTemplatePartialSpecialization:
                ...
                if (is_def) {
                    ...
                    if (auto* rd = dyn_cast<CXXRecordDecl>(d)) {
                        Decl* d1 = nullptr;
                        if (auto* sd = dyn_cast<
                                ClassTemplatePartialSpecializationDecl>(rd))
            ...
            case Decl::UnresolvedUsingTypename:
                if (auto* td = dyn_cast<TypedefNameDecl>(d)) {
                    ...
                    if (const Decl* d1 =
                            getAdjustedDecl(getTypeDecl(t, &specialization))) {
                        Usr usr1 = getUsr(d1);
                        IndexType& type1 = db->toType(usr1);
                        type->def.alias_of = usr1;
                        // Not visited template<class T> struct B {typedef A<T>
                        // t;};
                        if (specialization) {
   ...
   public:
    ...
    void MacroDefined(const Token& tok, const MacroDirective* md) override {
        ...
        if (IndexFile* db = param.consumeFile(fid)) {
            ...
            if (var.def.spell) var.declarations.push_back(*var.def.spell);
            ...
            if (var.def.detailed_name[0] == '\0') {
                ...
                var.def.hover = intern(
                    buf.count('\n') <= g_config->index.maxInitializerLines - 1
                        ? Twine("#define ", getSourceInRange(sm, lang, sr))
                              .str()
    ...
    void MacroExpands(const Token& tok, const MacroDefinition&, SourceRange sr,
        ...
        if (IndexFile* db = param.consumeFile(fid)) {
            ...
            var.uses.push_back({{fromTokenRange(sm, param.ctx->getLangOpts(),
                                                {sl, sl}, nullptr),
                                 Role::Dynamic}});
        }
    }
        ...
        class SkipProcessed : public ASTConsumer {
           ...
           public:
            ...
            bool shouldSkipFunctionBody(Decl* d) override {
                ...
                return !(g_config->index.multiVersion &&
                         param.useMultiVersion(fid)) &&
...
class IndexDiags : public DiagnosticConsumer {
   public:
    ...
    void HandleDiagnostic(DiagnosticsEngine::Level level,
        ...
        if (message.empty()) info.FormatDiagnostic(message);
    }
};
}  // namespace
...
void uniquify(std::vector<T>& a) {
    ...
    for (size_t i = 0; i < a.size(); i++)
        if (seen.insert(a[i]).second) a[n++] = a[i];
    a.resize(n);
}
...
IndexResult index(
    ...
    if (buf.size())
        for (auto& [filename, content] : remapped) {
            ...
            ci->getPreprocessorOpts().addRemappedFile(filename,
                                                      bufs.back().get());
        }
    ...
    std::string reason;
    {
        ...
        auto parse = [&]() {
            if (!action->BeginSourceFile(*clang,
    ...
    for (auto& it : param.uid2file) {
        ...
        for (auto& it : entry->usr2var) uniquify(it.second.uses);
