...
bool indexer_Parse(SemaManager* completion, WorkingFiles* wfiles,
    ...
    bool track =
        g_config->index.trackDependency > 1 ||
    ...
    if (reparse < 2) do {
                ...
                for (const auto& dep : prev->dependencies) {
                    if (auto mtime1 = lastWriteTime(dep.first.val().str())) {
                        if (dep.second < *mtime1) {
                            reparse = 2;
                            LOG_V(1)
                                << "timestamp changed for " << path_to_index
                                << " via " << dep.first.val().str();
                            break;
                        }
                    } else {
                        reparse = 2;
                        LOG_V(1) << "timestamp changed for " << path_to_index
                                 << " via " << dep.first.val().str();
                        break;
                    }
                }
            ...
            for (const auto& dep : dependencies) {
                ...
                IndexUpdate update =
                    IndexUpdate::createDelta(nullptr, prev.get());
                on_indexed->pushBack(std::move(update),
                                     request.mode != IndexMode::Background);
                if (entry.id >= 0) {
                    std::lock_guard lock2(project->mtx);
                    project->root2folder[entry.root].path2entry_index[path] =
                        entry.id;
                }
            }
            return true;
    ...
    if (deleted) {
        ...
        if (request.path != path_to_index)
            indexes.push_back(
                std::make_unique<IndexFile>(path_to_index, "", false));
    } else {
    ...
    if (loud || n_errs) {
        ...
        if (n_errs)
            msg += (" error:" + Twine(n_errs) + " " + first_error)
                       .toStringRef(tmp);
        if (LOG_V_ENABLED(1)) {
            msg += "\n ";
    ...
    for (std::unique_ptr<IndexFile>& curr : indexes) {
        ...
        if (!deleted)
            LOG_IF_S(INFO, loud)
                << "store index for " << path << " (delta: " << !!prev << ")";
        {
            std::lock_guard lock(getFileMutex(path));
            int loaded = vfs->loaded(path),
                retain = g_config->cache.retainInMemory;
            if (loaded)
                prev = rawCacheLoad(path);
            else
                prev.reset();
            ...
            if (g_config->cache.directory.size()) {
                ...
                if (deleted) {
                    ...
                    (void)sys::fs::remove(
                        appendSerializationFormat(cache_path));
                } else {
                    if (g_config->cache.hierarchicalPath)
                        sys::fs::create_directories(
                            sys::path::parent_path(cache_path,
            ...
            on_indexed->pushBack(
                IndexUpdate::createDelta(prev.get(), curr.get()),
                request.mode != IndexMode::Background);
            {
                std::lock_guard lock1(vfs->mutex);
...
void main_OnIndexed(DB* db, WorkingFiles* wfiles, IndexUpdate* update) {
    if (update->refresh) {
        LOG_S(INFO) << "loaded project. Refresh semantic highlight for all "
                       "working file.";
    ...
    if (update->files_def_update) {
        ...
        if (WorkingFile* wfile = wfiles->getFile(def_u.first.path)) {
            ...
            // request.path
            wfile->setIndexContent(g_config->index.onChange
                                       ? wfile->buffer_content
                                       : def_u.second);
...
void launchStdin() {
    ...
    std::thread([]() {
        ...
        while (true) {
            ...
            if (id.valid())
                LOG_V(2) << "receive RequestMessage: " << id.value << " "
                         << method;
            else
            ...
            on_request->pushBack(
                {id, std::move(method), std::move(message), std::move(document),
                 chrono::steady_clock::now() +
                     chrono::milliseconds(g_config ? g_config->request.timeout
                                                   : 0)});
...
void mainLoop() {
    ...
    while (true) {
        ...
        for (int i = 20; i--;) {
            ...
            if (update->files_def_update) {
                auto it =
                    path2backlog.find(update->files_def_update->first.path);
        ...
        if (completed != last_completed) {
            ...
            if (completed != enqueued) {
                ...
                int64_t last_idle =
                    stats.last_idle.load(std::memory_order_relaxed);
                WorkDoneProgressParam param;
                param.token = index_progress_token;
                param.value.kind = "report";
                param.value.message = (Twine(completed - last_idle) + "/" +
                                       Twine(enqueued - last_idle))
                                          .str();
        ...
        if (did_work) {
            ...
            if (backlog.empty())
                ...
                main_waiter->waitUntil(backlog[0].deadline, on_indexed,
                                       on_request);
        }
    }
