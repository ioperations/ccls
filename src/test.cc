    ...
    // Scan for OUTPUT:
    {
        ...
        for (StringRef line_with_ending : lines_with_endings) {
            ...
            if (line_with_ending.startswith("OUTPUT:")) {
                ...
                if (in_output) {
                    (*output_sections)[active_output_filename] =
                        active_output_contents;
                }

                // Try to tokenize OUTPUT: based one whitespace. If there is
                // more than one token assume it is a filename.
...
bool runIndexTests(const std::string& filter_path, bool enable_update) {
    ...
    getFilesInFolder(
        ...
        [&](const std::string& path) {
            ...
            parseTestExpectation(path, lines_with_endings, &text_replacer,
                                 &flags, &all_expected_output);
            ...
            for (const auto& entry : all_expected_output) {
                ...
                IndexFile* db =
                    findDbForPathEnding(expected_path, result.indexes);
                ...
                // Compare output via rapidjson::Document to ignore any
                ...
                if (actual == expected) {
                    ...
                    if (enable_update) {
                        printf(
                            "[Enter to continue - type u to update test, a to "
                            "update "
                            "all]");
                        char c = 'u';
                        if (!update_all) {
                        ...
                        if (update_all || c == 'u') {
                            // Note: we use |entry.second| instead of
                            // |expected_output| because |expected_output| has
                            // had text replacements applied.
