...
void MessageHandler::workspace_didChangeWatchedFiles(
    ...
    for (auto& event : param.changes) {
        ...
        if ((g_config->cache.directory.size() &&
            ...
            return;
        for (std::string cur = path; cur.size();
             cur = sys::path::parent_path(cur))
            if (cur[0] == '.') return;

        switch (event.type) {
            case FileChangeType::Created:
            case FileChangeType::Changed: {
                IndexMode mode = wfiles->getFile(path) ? IndexMode::Normal
...
void MessageHandler::workspace_didChangeWorkspaceFolders(
    ...
    for (const WorkspaceFolder& wf : param.event.removed) {
        ...
        auto it = llvm::find_if(g_config->workspaceFolders, [&](auto& folder) {
            return folder.first == root;
        });
...
done_add:
    ...
    if (g_config->workspaceSymbol.sort &&
        ...
        for (auto& cand : cands)
            longest = std::max(
                longest,
                int(db->getSymbolName(std::get<2>(cand), true).size()));
