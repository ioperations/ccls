...
void MessageHandler::textDocument_codeAction(CodeActionParam& param,
                                             ReplyOnce& reply) {
    if (!g_config->client.codeActionProvider) {
        reply(JsonNull());
        return;
    }
    WorkingFile* wf =
    ...
    for (auto [sym, refcnt] : file->symbol2refcnt) {
        ...
        switch (sym.kind) {
            case Kind::Func: {
                ...
                add("ref", {sym.usr, Kind::Func, "uses"}, sym.range,
                    func.uses.size(), base_uses.empty());
                if (base_uses.size())
                    add("b.ref", {sym.usr, Kind::Func, "bases uses"}, sym.range,
                        base_uses.size());
                if (derived_uses.size())
                    add("d.ref", {sym.usr, Kind::Func, "derived uses"},
                        sym.range, derived_uses.size());
                if (base_uses.empty())
            ...
            case Kind::Type: {
                ...
                add("ref", {sym.usr, Kind::Type, "uses"}, sym.range,
                    type.uses.size(), true);
            ...
            case Kind::Var: {
                ...
                if (!def ||
                    (def->is_local() && !g_config->codeLens.localVariables))
                    continue;
                add("ref", {sym.usr, Kind::Var, "uses"}, sym.range,
                    var.uses.size(), def->kind != SymbolKind::Macro);
                break;
            }
            case Kind::File:
            case Kind::Invalid:
