    ...
    for (const auto& chunk : ccs) {
        switch (chunk.Kind) {
            ...
            case CodeCompletionString::CK_Placeholder:
                ...
                // call argument. Intentional fallthrough here.
            case CodeCompletionString::CK_CurrentParameter: {
                // A piece of text that describes the parameter that corresponds
                // to the code-completion location within a function call,
                // message send, macro invocation, etc.
   ...
   public:
    ...
    void ProcessOverloadCandidates(Sema& s, unsigned currentArg,
        ...
        for (unsigned i = 0; i < numCandidates; i++) {
            ...
            // may be long in some cases (e.g. when 'T' is substituted with
            // 'std::string', we would get 'std::basic_string<char>').
            ...
            ls_sig.documentation =
                rc ? rc->getBriefText(s.getASTContext()) : "";
            for (const auto& chunk : *ccs) switch (chunk.Kind) {
                    ...
                    case CodeCompletionString::CK_CurrentParameter: {
                        ...
                        ls_sig.parameters.push_back(
                            {{off, (int)ls_sig.label.size()}});
                        break;
                    }
                    case CodeCompletionString::CK_Optional:
                        buildOptional(*chunk.Optional, ls_sig.label,
                                      ls_sig.parameters);
                        break;
                    case CodeCompletionString::CK_VerticalSpace:
                        break;
...
void MessageHandler::textDocument_signatureHelp(
    ...
    SemaManager::OnComplete callback = [reply, path, begin_pos, buffer_line](
                                           CodeCompleteConsumer* optConsumer) {
