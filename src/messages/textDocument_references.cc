...
void MessageHandler::textDocument_references(JsonReader& reader,
            ...
            withEntity(db, sym, [&](const auto& entity) {
                ...
                for (auto& def : entity.def)
                    if (def.spell) {
                        ...
                        if (param.base)
                            for (Usr usr :
                                 make_range(def.bases_begin(), def.bases_end()))
                                if (!seen.count(usr)) {
                                    seen.insert(usr);
                                    stack.push_back(usr);
                                }
    ...
    if (result.empty()) {
        // |path| is the #include line. If the cursor is not on such line but
        // line = 0, use the current filename.
        std::string path;
