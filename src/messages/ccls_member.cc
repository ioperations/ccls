...
struct Param : TextDocumentPositionParam {
    ...
    int levels = 1;
    // If Kind::Func and the point is at a type, list member functions instead
    // of member variables.
    Kind kind = Kind::Var;
...
bool expand(MessageHandler* m, Out_cclsMember* entry, bool qualified,
    ...
    if (levels > 0) {
        ...
        while (stack.size()) {
            ...
            if (def->alias_of) {
                const QueryType::Def* def1 =
                    m->db->getType(def->alias_of).anyDef();
                Out_cclsMember entry1;
                ...
                if (def1 && def1->spell) {
                    ...
                    // Builtin types have no declaration but the typedef
                    // declaration itself is useful.
                    if (std::optional<Location> loc =
                    ...
                    for (Usr usr : def.funcs)
                        if (seen1.insert(usr).second) {
                            ...
                            if (const QueryFunc::Def* def1 = func1.anyDef()) {
                                ...
                                entry1.fieldName = def1->name(false);
                                if (def1->spell) {
                                    if (auto loc = getLsLocation(
                                            m->db, m->wfiles, *def1->spell))
                                        entry1.location = *loc;
                ...
                for (auto& def : type->def)
                    for (auto it : def.vars)
                        if (seen1.insert(it.first).second) {
                            ...
                            if (!var.def.empty())
                                doField(m, entry, var, it.second, qualified,
                                        levels - 1);
                        }
            }
        }
    ...
    if (param.id.size()) {
        ...
        for (SymbolRef sym : findSymbolsAtLocation(wf, file, param.position)) {
            switch (sym.kind) {
                ...
                case Kind::Type:
                    result =
                        buildInitial(this, sym.kind, sym.usr, param.qualified,
                                     param.levels, param.kind);
                    break;
                case Kind::Var: {
                    const QueryVar::Def* def = db->getVar(sym).anyDef();
                    if (def && def->type)
                        result = buildInitial(this, Kind::Type, def->type,
