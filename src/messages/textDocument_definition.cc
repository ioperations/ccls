...
void MessageHandler::textDocument_definition(TextDocumentPositionParam& param,
    ...
    if (result.empty()) {
        ...
        if (!range) {
            ...
            // substring, we use the tuple <length difference, negative
            // position, not in the same file, line distance> to find the best
            // match.
            ...
            auto fn = [&](SymbolIdx sym) {
                ...
                if (Maybe<DeclRef> dr = getDefinitionSpell(db, sym)) {
                    ...
                    // Update the score with qualified name if the qualified
                    // name occurs in |name|.
