...
void filterCandidates(CompletionList& result, const std::string& complete_text,
    ...
    auto finalize = [&]() {
        ...
        for (auto& item : items) {
            ...
            if (edits.size() && edits[0].range.end == begin_pos) {
                ...
                if (start.line == begin_pos.line) {
                    overwrite_len = std::max(overwrite_len,
        ...
        for (auto& item : items) {
            ...
            if (overwrite_len > 0) {
                ...
                std::string orig = buffer_line.substr(overwrite_begin.character,
                                                      overwrite_len);
                if (edits.size() && edits[0].range.end == begin_pos &&
                    edits[0].range.start.line == begin_pos.line) {
                    int cur_edit_len = edits[0].range.end.character -
    ...
    if (complete_text.size()) {
        ...
        for (CompletionItem& item : items) {
            ...
            item.score_ =
                reverseSubseqMatch(complete_text, filter, sensitive) >= 0
    ...
    for (const auto& chunk : ccs) {
        ...
        switch (kind) {
            ...
            case CodeCompletionString::CK_CurrentParameter:
                // This should never be present while collecting completion
                // items.
        ...
        for (auto i = first; i < out.size(); ++i) {
            ...
            if (kind == CodeCompletionString::CK_Placeholder) {
                ...
                out[i].textEdit.newText +=
                    ("${" + Twine(out[i].parameters_.size()) + ":" + text + "}")
                        .str();
        ...
        for (unsigned i = 0; i != numResults; i++) {
            ...
            if (r.Declaration) {
                ...
                if (k == Decl::FunctionTemplate) {
                    ...
                    if (fd->getTemplatedDecl()->getKind() ==
                    ...
                    if (s.size()) {
                        ...
                        if (s.back() == '(' || s.back() == '<')
                            s.pop_back();
                                 ...
                                 !s.compare(s.size() - 2, 2, "()"))
                            s.resize(s.size() - 2);
                    }
                } else if (ls_items[j].insertTextFormat ==
            ...
            for (const FixItHint& fixIt : r.FixIts) {
                ...
                TextEdit ls_edit = ccls::toTextEdit(ast.getSourceManager(),
                                                    ast.getLangOpts(), fixIt);
                for (size_t j = first_idx; j < ls_items.size(); j++)
